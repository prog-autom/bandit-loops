...
\begin{statement}
  Пусть  $p=1$ и TS работает в режиме c фиксированными лидереми начиная с какого-то момента времени $\tau$. Тогда при $w \geq 0 : \lim_{t \to \infty} \|\mu_t - \mu_0 \|_2 = \infty.$
...
\begin{figure}[h]
  \begin{multicols}{2}
    \hfill
    \includegraphics[width=9cm]{norm_interest.pdf}
    \hfill
    \caption{Логарифм нормы интереса на очередном шаге рекомендации.}
    \label{fig1}
    \hfill
    \includegraphics[width=9cm]{rewards.pdf}
    \hfill
...
\begin{figure}[h]
  \begin{multicols}{2}
    \hfill
    \includegraphics[width=9cm]{var_norm_interest.pdf}
    \hfill
    \caption{Разброс логарифма нормы интереса от шага.}
    \label{fig3}
    \hfill
    \includegraphics[width=9cm]{compare_random_and_ts.pdf}
    \hfill
...
%   \begin{multicols}{2}
%     \hfill
%     \includegraphics[width=8cm]{3d_var_wp_max_interest.pdf}
%     \hfill
...
\[
  \mathsf{E}(\mu_{t} - \mu_{t-1}) = \mathsf{E}\delta_t \cdot \mathsf{E}(\mathsf{E}(\mathsf{E}(c_t|s_t, q_t))) > 0.
\]
Поэтому существет $m > 0$ такое, что 
\[
  \mathsf{E}(\mu_{t} - \mu_{t-1}) = \mathsf{E}\delta_t \cdot \mathsf{E}(\mathsf{E}(\mathsf{E}(c_t|s_t, q_t))) > m > 0.
\]
Тогда 
